...
define(["js/directive/panel/qsPanel",
    ...
    "js/service/clientRepository"], function () {

    return ['$scope', '$filter', '$location', "GridData", "GridCommon", "SweetAlert", "clientRepository", function ($scope, $filter, $location, GridData, GridCommon, SweetAlert, clientRepository) {
        ...
        // grid对应配置json
        $scope.interface.url = 'config/client/clientPanel.json';

        $scope.items = [];
        $scope.clientSearchJsonUrl = 'config/client/clientSearch.json';
        $scope.clientCrudJsonUrl = 'config/client/clientCrud.json';
        ...
        $scope.fresh = function () {
            clientRepository.getAll().then(function (items) {
                $scope.items = items;
        ...
        $scope.addRowFuc = function () {
            ...
            $location.path('dashboard/client/client-add');
        }
        ...
        $scope.delRowFuc = function () {
            ...
            SweetAlert.swal({
                ...
                if (isConfirm) {
                    ...
                    angular.forEach($scope.interface.selectedItems, function (selectedItem) {
                        clientRepository.remove(selectedItem).then(function () {
                            delCount++;
         ...
         $scope.modifyRowFuc = function () {
             if ($scope.interface.selectedItems.length <= 0) {
                 ...
                 GridData.setCurPage($scope.interface.curPage);
                 $location.path('dashboard/client/client-modify/' + $scope.interface.selectedItems[0].id);
             }
         }
         ...
         $scope.viewRowFuc = function () {
             if ($scope.interface.selectedItems.length <= 0) {
                 ...
                 GridData.setCurPage($scope.interface.curPage);
                 $location.path('dashboard/client/client-view/' + $scope.interface.selectedItems[0].id);
             }
         }
         $scope.interface.edit = function (id) {
             $location.path('dashboard/client/client-modify/' + id);
             GridData.setCurPage($scope.interface.curPage);
         }

         $scope.interface.view = function (id) {
             $location.path('dashboard/client/client-view/' + id);
             GridData.setCurPage($scope.interface.curPage);
         }
         ...
         $scope.interface.del = function (item) {
             SweetAlert.swal({
                 ...
                 if (isConfirm) {
                     clientRepository.remove(item).then(function () {
                         SweetAlert.swal('删除成功！');
                         $scope.fresh();
                     });
                 }
